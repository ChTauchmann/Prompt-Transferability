...
class activate_neuronPromptT5Formatter(BasicFormatter):
    ...
    def process(self, data, config, mode, *args, **params):
        ...
        for ins in data:
            ...
            sent = self.tokenizer.encode("<extra_id_0>", add_special_tokens = False)
            #sent = self.tokenizer.encode("<extra_id_0> this is a good", add_special_tokens = False)
            #if len(sent) > max_len:
            #    sent = sent[:max_len]
