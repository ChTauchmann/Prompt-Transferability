...
logger = logging.get_logger('root.modeling_t5')
...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, prompt_tokens=None):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            if prompt_tuning:
                original = self.embed_tokens(input_ids * (input_ids >= 0).int()) * (input_ids >= 0).int().unsqueeze(2)
                #prompt = self.prompt_tokens(input_ids * (input_ids < 0).int() * -1) * (input_ids < 0).int().unsqueeze(2)
                prompt_ids = - (input_ids+1)
                prompt = self.prompt_tokens(prompt_ids * (prompt_ids >= 0).int()) * (prompt_ids >= 0).int().unsqueeze(2)

                inputs_embeds = original + prompt
            else:
...
class   T5ForConditionalGeneration(T5PreTrainedModel):
    _keys_to_ignore_on_load_missing = [
        r"encoder\.embed_tokens\.weight",
        r"decoder\.embed_tokens\.weight",
        r"encoder\.prompt_tokens\.weight",
        r"decoder\.prompt_tokens\.weight",
        r"lm_head\.weight",
    ]
    _keys_to_ignore_on_load_unexpected = [
        r"decoder\.block\.0\.layer\.1\.EncDecAttention\.relative_attention_bias\.weight",
    ...
    def __init__(self, config):
        ...
        self.shared = nn.Embedding(config.vocab_size, config.d_model)
        # self.prompt_embedding = copy.deepcopy(self.shared)
        #self.prompt_embedding = nn.Embedding(100, config.d_model)
        ...
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        self.encoder = T5Stack(encoder_config, self.shared, self.prompt_embeddings)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = T5Stack(decoder_config, self.shared, self.prompt_embeddings)
    ...
    def init_prompt_emb(self, init_ids):
        prompt_weights = self.shared(init_ids).detach()
        self.prompt_embedding.weight.data = prompt_weights
        logger.info("init_prompt_done, check_if_requires_grad")
